...
function updateImportPaths(content, filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (importMatch) {
      ...
      // Skip if it's already correct or is a feature-internal import
      if (oldPath.startsWith('../../features/') || oldPath.startsWith('../')) {
        // Check if it's a feature-internal import (within the same feature)
        const isInternal = !oldPath.includes('components') && 
                           !oldPath.includes('services') && 
                           !oldPath.includes('contexts') &&
                           !oldPath.includes('utils') &&
                           !oldPath.includes('data');
        
        if (isInternal) {
          continue; // Skip internal feature imports
        }
      }
      
      // Update the path
      const newPath = correctPath + oldPath.replace(/^(\.\.\/)+/, '');
