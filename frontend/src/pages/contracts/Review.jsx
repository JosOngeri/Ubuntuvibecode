...
export default function ContractReview({ standalone = true }) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [invoicesResponse] = await Promise.all([contractorAPI.getInvoices()]);
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr,0.8fr]">
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            Due: {invoice.due ? new Date(invoice.due).toLocaleDateString() : 'N/A'}
                          </div>
                        </div>
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold ${['approved', 'paid'].includes(String(invoice.status).toLowerCase()) ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200' : 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200'}`}
                        >
                          {invoice.status}
                        </span>
                      </div>
                  ...
                  {milestones.map(milestone => (
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold ${String(milestone.status).toLowerCase().includes('approved') ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200' : 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200'}`}
                        >
                          {milestone.status}
                        </span>
                      </div>
                      ...
                      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                        Proof file: {milestone.proofName}
                      </p>
                      <div className="mt-4 flex flex-wrap gap-3">
                        <button
          ...
          <aside className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                  ...
                  <div className="mt-1 text-sm text-slate-800 dark:text-slate-200">
                    KES {Number(selectedMilestone.amount).toLocaleString()}
                  </div>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                  <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">
                  ...
                  <div className="mt-1 text-sm text-slate-800 dark:text-slate-200">
                    {selectedMilestone.due
                      ? new Date(selectedMilestone.due).toLocaleDateString()
                      : 'N/A'}
                  </div>
                </div>
                <button
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-950 p-4 text-white dark:bg-white dark:text-slate-950">
              ...
              <div className="mt-1 text-sm text-slate-300 dark:text-slate-700">
                {backendInvoices.length} invoice record(s) loaded.
              </div>
            </div>
          </aside>
        </div>
