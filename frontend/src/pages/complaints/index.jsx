...
export default function ComplaintsPage({ standalone = true }) {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const r = await api.get('/complaints').catch(() => ({ data: [] }));
  ...
  const create = async () => {
    try {
      ...
      toast.success('Filed');
      setShowForm(false);
      setForm({ ...form, category: '', description: '' });
      fetchAll();
    } catch (e) {
  ...
  const openCount = complaints.filter(c =>
    ['open', 'acknowledged', 'investigating'].includes(c.status)
  ).length;
    ...
    <div>
      <div className="page-header mb-6">
        ...
        <p className="page-subtitle">
          Manage guest complaints and employee grievances with SLA tracking.
        </p>
      </div>
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        >
          <div className="stat-card">
            ...
            <span className="stat-value text-red-600">{openCount}</span>
            <p className="text-xs text-blue-500 mt-1">Click to view →</p>
          </div>
        </Card>
        <Card
      ...
      {showForm && (
        <Card className="mb-4">
          ...
          <textarea
            className="form-input text-sm w-full mt-3"
            rows={3}
      ...
      {loading ? (
        ...
        <Card>
          <div className="text-center py-8 text-slate-500">No complaints.</div>
        </Card>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={'px-2 py-0.5 rounded text-xs font-medium ' + UC[c.urgency]}>
                      {c.urgency}
                    </span>
                    <span className={'px-2 py-0.5 rounded text-xs font-medium ' + SC[c.status]}>
                      {c.status}
                    </span>
                    {c.slaDeadline &&
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                    {c.guestName && (
                      <span>
                        ...
                        {c.guestName}
                        {c.guestRoom && ' · Rm ' + c.guestRoom}
                      </span>
                    )}
                    {c.department && (
                      <span>
                        <BsBuilding className="inline mr-1" size={10} />
                        {c.department}
                      </span>
                    )}
                    <span>
                ...
                <div className="mt-4 pt-4 border-t" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="flex gap-2">
                    ...
                    {c.status === 'investigating' && (
                      <Button variant="primary" size="sm" onClick={() => openResolveModal(c._id)}>
                        Resolve
                      </Button>
                    )}
                    {c.status === 'resolved' && (
                      <Button variant="secondary" size="sm" onClick={() => close(c._id)}>
                        Close
                      </Button>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              value={resolveForm.prevention}
              onChange={e => setResolveForm({ ...resolveForm, prevention: e.target.value })}
            />
          </div>
