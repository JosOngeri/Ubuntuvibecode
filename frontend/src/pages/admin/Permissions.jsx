            ...
            {activeTab === 'role' && canEditRolePermissions && (
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-500 mt-2">
                  Editing default permissions for all users with the <strong>{selectedRole}</strong> role.
                </p>
              </div>
            )}
              ...
              <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
                ...
                <div className="space-y-2">
                  {activeOverrides.map(override => (
                    <div key={override.id} className="flex items-center justify-between p-2 bg-white dark:bg-slate-800 rounded border">
                      <div>
                        <span className="font-medium">{formatPermissionLabel(override.permissionKey)}</span>
                  ...
                  {expandedGroups[groupKey] && (
                    <div className="p-4 space-y-2">
                      {Object.entries(group.permissions).map(([permKey, permValue]) => {
                        const isGranted = activeTab === 'role' 
                          ? rolePermissions.includes(permValue)
                          : userPermissions?.effectivePermissions?.includes(permValue) || userPermissions?.rolePermissions?.includes(permValue);
                        const isSelected = selectedPermissions.includes(permValue);
                        
                        return (
                          <label key={permKey} className="flex items-center gap-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded cursor-pointer">
                            <input
                              type="checkbox"
                              checked={isSelected}
