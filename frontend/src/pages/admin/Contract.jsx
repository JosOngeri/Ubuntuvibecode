...
export default function Contract({ standalone = true }) {
  ...
  const filteredContracts = useMemo(() => {
    let filtered = contracts.filter(contract => {
      const empName = String(
        getEmployeeName(contract.employee || contract.employee_id)
      ).toLowerCase();
      ...
      const matchesStatus =
        statusFilter === 'all' || (contract.status || 'active') === statusFilter;
  ...
  const contractsByDept = useMemo(() => {
    ...
    contracts.forEach(c => {
      const emp = employees.find(
        e =>
          String(e.id) === String(c.employee || c.employee_id) ||
          String(e._id) === String(c.employee || c.employee_id)
      );
  ...
  const handleExportReport = async () => {
    await downloadPdfReport({
      ...
      columns: [
        ...
        {
          label: 'Start Date',
          getValue: row => (row.startDate ? new Date(row.startDate).toLocaleDateString() : ''),
        },
        {
          label: 'End Date',
          getValue: row => (row.endDate ? new Date(row.endDate).toLocaleDateString() : 'N/A'),
        },
  ...
  const columns = [
    ...
    { key: 'title', label: 'Contract Title', sortable: true },
    {
      ...
      label: 'Start Date',
      sortable: true,
      render: (_, row) => (row.startDate ? new Date(row.startDate).toLocaleDateString() : 'N/A'),
    },
    {
      key: 'endDate',
      label: 'End Date',
      sortable: true,
      render: (_, row) => (row.endDate ? new Date(row.endDate).toLocaleDateString() : 'N/A'),
    },
    {
      key: 'status',
      label: 'Status',
    ...
    <div>
      <div className="page-header mb-6">
        ...
        <p className="page-subtitle">
          Track employee contracts, view terms, and generate compliance reports.
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="flex flex-col items-center">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-6 w-full text-left">
                Status Distribution
              </h3>
              {stats.total === 0 ? (
                ...
                <>
                  <div
                    className="w-48 h-48 rounded-full shadow-inner mb-6"
                  ...
                  <div className="w-full flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold">
                        {activePct.toFixed(1)}% ({stats.active})
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold">
                        {expiredPct.toFixed(1)}% ({stats.expired})
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold">
                        {terminatedPct.toFixed(1)}% ({stats.terminated})
                      </span>
                    </div>
                  </div>
                </>
                ...
                {contractsByDept.length === 0 ? (
                  ...
                  contractsByDept.map(([dept, count]) => (
                    <div key={dept} className="flex items-center gap-4">
                      <div
                        className="w-32 text-sm font-medium text-slate-700 dark:text-slate-300 truncate"
                        title={dept}
                      >
                        {dept}
                      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              onChange={handleChange}
              rows={5}
              placeholder="Specify the terms of the contract..."
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Attach a signed copy of the contract (PDF, DOC, DOCX).
            </p>
          </div>
      ...
      >
        {selectedContract && (
          <div className="space-y-4">
            <div>
              <strong className="text-slate-600 dark:text-slate-400">Employee:</strong>{' '}
              <span className="block mt-1 text-lg">
                {getEmployeeName(selectedContract.employee || selectedContract.employee_id)}
              </span>
            </div>
            <div>
              <strong className="text-slate-600 dark:text-slate-400">Title:</strong>{' '}
              <span className="block mt-1">{selectedContract.title}</span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <strong className="text-slate-600 dark:text-slate-400">Start Date:</strong>{' '}
              ...
              <div>
                <strong className="text-slate-600 dark:text-slate-400">End Date:</strong>{' '}
                <span className="block mt-1">
            ...
            <div>
              ...
              <span
                className={`inline-block mt-1 px-3 py-1 rounded-full text-xs font-medium uppercase ${getStatusColor(selectedContract.status)}`}
              >
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
              ...
              <div className="mt-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg whitespace-pre-wrap text-sm">
            ...
            {selectedContract.documentPath && (
              <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                ...
                <a
                  href={`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${selectedContract.documentPath.startsWith('/') ? '' : '/'}${selectedContract.documentPath}`}
                  target="_blank"
