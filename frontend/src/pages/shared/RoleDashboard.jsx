...
const RoleDashboard = ({ role = 'employee', standalone = true }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (role === 'admin' || role === 'manager') {
          const [empRes, payrollRes, kpiRes, attRes, allAttRes, leaveRes] = await Promise.all([
            employeeAPI.getAll().catch(() => ({ data: [] })),
            payrollAPI.getPayments().catch(() => ({ data: [] })),
          ...
          const emps = Array.isArray(empRes?.data) ? empRes.data : Array.isArray(empRes) ? empRes : [];
          const pays = Array.isArray(payrollRes?.data) ? payrollRes.data : Array.isArray(payrollRes) ? payrollRes : [];
          const ks = Array.isArray(kpiRes?.data) ? kpiRes.data : Array.isArray(kpiRes) ? kpiRes : [];
          const att = Array.isArray(attRes?.data) ? attRes.data : Array.isArray(attRes) ? attRes : [];
          const allAtt = Array.isArray(allAttRes?.data) ? allAttRes.data : Array.isArray(allAttRes) ? allAttRes : [];
          const lvs = Array.isArray(leaveRes?.data) ? leaveRes.data : Array.isArray(leaveRes) ? leaveRes : [];
          ...
          const [teamRes, attRes] = await Promise.all([
            api
              .get('/roles/supervisor/team')
              .catch(() => ({ data: { employees: [], dailyLabourers: [] } })),
          ...
          const supervisors = Array.isArray(supRes?.data) ? supRes.data : [];
      ...
      return [
        {
          key: 'employees',
          label: 'Employees',
          value: teamMembers.filter(m => m.type !== 'Daily Labourer').length,
          icon: BsPeople,
        },
        {
          key: 'labourers',
          label: 'Daily Labourers',
          value: teamMembers.filter(m => m.type === 'Daily Labourer').length,
          icon: BsPeople,
        },
        {
        ...
        },
        {
          key: 'absent',
          label: 'Absent Today',
          value: recentAttendance.filter(a => a.status === 'absent').length,
          icon: BsXCircle,
          onClick: () => navigate('/manager/attendance'),
        },
      ];
      ...
      return [
        {
          ...
          value: contractorStats.activeProjects,
          icon: BsBriefcase,
        ...
        },
        {
          ...
          value: contractorStats.pendingInvoices,
        ...
        },
        {
          key: 'delivery',
          label: 'Delivery Rate',
          value: `${contractorStats.deliveryRate}%`,
          icon: BsGraphUp,
        ...
        },
        {
          ...
          value: `KES ${contractorStats.totalEarnings?.toLocaleString() || 0}`,
      ...
      return [
        {
          ...
          description: `${pendingLeaves} pending`,
          icon: BsCalendarCheck,
          route: '/admin/leaves',
        },
        {
          ...
          description: `${pendingPayroll} pending`,
          icon: BsCreditCard,
          route: '/admin/payroll',
        },
        {
      ...
      return [
        {
          key: 'leaves',
          label: 'Review Leaves',
          description: `${pendingLeaves || 3} pending`,
          icon: BsCheckCircle,
        ...
        },
        {
          ...
          icon: BsCreditCard,
          route: '/payroll/disburse',
        },
        {
          key: 'leave-overview',
          label: 'Leave Overview',
          description: 'Team leave balance',
          icon: BsCalendarCheck,
    ...
    if (role === 'contractor')
      return [
        ...
        },
        {
          key: 'invoices',
          label: 'View Invoices',
          description: `${contractorStats.pendingInvoices} pending`,
          icon: BsFileEarmarkText,
        ...
        },
        {
          key: 'projects',
          label: 'My Projects',
          description: `${contractorStats.activeProjects} active`,
          icon: BsBriefcase,
      ...
      return [
        {
          ...
          description: `${presentDays} days recorded`,
          icon: BsCalendarCheck,
          route: '/daily-labour/attendance',
        },
        {
          key: 'payments',
          label: 'View Payments',
          description: `${pendingPayments} pending`,
          icon: BsWallet,
          route: '/daily-labour/payments',
        },
      ];
    return [];
  };
                  ...
                  <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500">
                        {mySupervisor.type === 'permanent'
                          ? 'Permanent allocation'
                          : `Allocation type: ${mySupervisor.type}`}
                      </div>
                    </div>
                    <button
                ...
                {recentActivity.map(activity => (
                  ...
                  >
                    <div className="p-2 bg-orange-100 dark:bg-orange-900/30 rounded-lg">
                      ...
                      {activity.type === 'kpi' && (
                        <BsBullseye size={18} className="text-orange-600 dark:text-orange-400" />
                      )}
                    </div>
                    <div className="flex-1">
                      <div className="text-sm font-medium text-slate-900 dark:text-white">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-900 dark:text-white">
                    {contractorStats.upcomingDeadlines}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-3 text-sm">
                {[
                  {
                    label: 'Daily Rate',
                    value: labourer?.daily_rate
                      ? `KES ${parseFloat(labourer.daily_rate).toLocaleString()}`
                      : 'N/A',
                  },
          ...
          {role === 'supervisor' && (
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-2">
                <button
                  onClick={() => navigate('/manager/attendance')}
                  className="w-full flex items-center gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors text-left"
                >
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-teal-500 to-teal-600 rounded-xl p-6 text-white">
        <h2 className="text-2xl font-bold mb-2">
          {role === 'admin' ? 'Employee Management' : 'Team Members'}
        </h2>
        <p className="text-teal-100">
          {employees.length} {role === 'admin' ? 'employees' : 'team members'} total
        </p>
      </div>
      <DataTable
        columns={[
        ...
        data={recentEmployees.map(emp => ({
          ...
          position: emp.position || emp.jobTitle || emp.job_title || '-',
          type: emp.employmentType || emp.employment_type || 'Permanent',
  ...
  const renderAttendanceTab = () => {
    ...
    const attRecords = role === 'admin' || role === 'manager' ? allAttendance : recentAttendance;
    return (
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-6 text-white">
          <h2 className="text-2xl font-bold mb-2">Attendance Overview</h2>
          <p className="text-green-100">{attRecords.length} record(s) — latest first</p>
        </div>
        <DataTable
          columns={[
            ...
            },
            {
              ...
              label: 'Hours',
              render: val => (val ? Number(val).toFixed(2) + ' hrs' : '-'),
            },
          ]}
          data={attRecords.map(a => ({
            id: a.id,
            employeeName:
              a.employeeName ||
              (a.employeeId?.firstName
                ...
                : '-'),
            attendanceDate: a.attendanceDate || a.attendance_date,
            status: a.status,
            shift: a.shift || '-',
            checkIn: a.checkIn || a.check_in,
            checkOut: a.checkOut || a.check_out,
            totalHoursWorked: a.totalHoursWorked || a.total_hours_worked,
          }))}
          onRowClick={row =>
            navigate(`/${role === 'admin' ? 'admin' : 'manager'}/attendance/${row.id}`)
          }
        />
        <div className="flex justify-end">
          <button
            onClick={() => navigate(role === 'admin' ? '/admin/attendance' : '/manager/attendance')}
            className="text-sm text-orange-500 hover:text-orange-600 font-medium"
          >
            Manage attendance →
          </button>
  ...
  const renderPayrollTab = () => (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl p-6 text-white">
        ...
        <p className="text-amber-100">{payrolls.length} record(s) — latest first</p>
      </div>
      <DataTable
        columns={[
          ...
          { key: 'period', label: 'Period', sortable: true },
          {
            ...
            label: 'Gross Pay',
            sortable: true,
            render: val => (val ? `KES ${Number(val).toLocaleString()}` : '-'),
          },
          {
            key: 'netPay',
            label: 'Net Pay',
          ...
          },
          {
            ...
            render: val => (
              <span
                className={`px-2 py-1 text-xs font-medium rounded-full ${
                  String(val).toLowerCase() === 'paid' || String(val).toLowerCase() === 'approved'
                    ? 'bg-green-100 text-green-700'
                    : String(val).toLowerCase() === 'draft' ||
        ...
        data={payrolls.map(p => ({
          ...
          period:
            ...
            (p.created_at
              ...
              : '-'),
          grossPay: p.gross_pay || p.grossPay || p.amount,
          netPay: p.net_pay || p.netPay,
          status: p.status,
        }))}
        onRowClick={() => navigate('/admin/payroll')}
      />
      <div className="flex justify-end">
        <button
          onClick={() => navigate('/admin/payroll')}
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl p-6 text-white">
        ...
        <p className="text-purple-100">
          {kpis.length} KPI record(s) — avg score {realAvgKPI}%
        </p>
      </div>
      <DataTable
        columns={[
          ...
          { key: 'targetScore', label: 'Target', sortable: true },
          {
            key: 'finalScore',
            label: 'Score',
            sortable: true,
        ...
        data={kpis.map(k => ({
          id: k.id || k._id,
          employeeName:
            `${k.first_name || k.firstName || ''} ${k.last_name || k.lastName || ''}`.trim() ||
            k.employee_name ||
            '-',
          title: k.definition_title || k.title || k.kpiTitle || '-',
          targetScore: k.target_score || k.targetScore || '-',
          finalScore: k.final_score ?? k.finalScore ?? null,
          status: k.status || '-',
        }))}
        onRowClick={() => navigate(role === 'admin' ? '/admin/performance' : '/kpi/manage')}
      />
      <div className="flex justify-end">
        <button
          onClick={() => navigate(role === 'admin' ? '/admin/performance' : '/kpi/manage')}
          className="text-sm text-orange-500 hover:text-orange-600 font-medium"
        >
          Manage KPIs →
        </button>
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl p-6 text-white">
        ...
        <p className="text-amber-100">
          {leaves.length} leave request(s) — {pendingLeaves} pending
        </p>
      </div>
      <DataTable
        columns={[
  ...
  const renderEmployeeLeavesTab = () => (
    <div className="space-y-6">
      ...
      <button
        onClick={() => navigate('/leave/request')}
        className="w-full px-4 py-3 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition"
      >
        Request New Leave
      </button>
    </div>
  ...
  const renderAnalyticsTab = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Employees', value: employees.length, reportType: 'employee' },
          { label: 'Present Today', value: todayAttendance.length, reportType: 'attendance' },
          { label: 'Pending Leaves', value: pendingLeaves, reportType: 'leave' },
          { label: 'Avg KPI Score', value: `${realAvgKPI}%`, reportType: 'kpi' },
        ].map(({ label, value, reportType }) => (
          <div
            key={label}
            className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-slate-200 dark:border-slate-700 text-center cursor-pointer hover:shadow-lg hover:border-orange-300 transition-all duration-200"
            onClick={() => navigate('/reports?tab=' + reportType)}
          >
            <div className="text-2xl font-bold text-slate-900 dark:text-white">{value}</div>
            <div className="text-xs text-slate-500 mt-1">{label}</div>
            <div className="text-xs text-orange-500 mt-2">Click to view →</div>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {paymentData.map(p => (
                ...
                >
                  <td className="px-4 py-3 text-slate-700 dark:text-slate-300">
                    {new Date(p.payment_date || p.date || p.created_at).toLocaleDateString(
                      'en-KE',
  ...
  const getTabs = () => {
    const allTabs = {
      ...
      employeeLeaves: {
        ...
        label: 'Leaves',
        icon: BsCalendarCheck,
        render: renderEmployeeLeavesTab,
      },
