...
export default function AssetsPage({ standalone = true }) {
  ...
  const create = async () => {
    try {
      ...
      toast.success('Added');
      setShowForm(false);
      setForm({ name: '', type: 'equipment', serialNumber: '', condition: 'good' });
      fetchAll();
    } catch (e) {
  ...
  const assign = async () => {
    ...
    try {
      await api.put('/assets/' + assignId + '/assign', { employeeId: assignEmpId });
  ...
  const content = (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        >
          <div className="stat-card">
            ...
            <span className="stat-value text-blue-600">{totalAssigned}</span>
            <p className="text-xs text-blue-500 mt-1">Click to view →</p>
          </div>
        </Card>
        <Card
        ...
        >
          <div className="stat-card">
            ...
            <span className="stat-value text-green-600">{totalAvailable}</span>
            <p className="text-xs text-blue-500 mt-1">Click to view →</p>
          </div>
        </Card>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <div className="text-center py-8 text-slate-500">No assets registered.</div>
        </Card>
      ) : (
        <div className="grid gap-3">
          {filteredAssets.map(a => (
            <Card key={a._id} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {a.serialNumber || 'No S/N'} · {a.condition} ·{' '}
                    <button
                      onClick={() => setStatusFilter(a.status === statusFilter ? 'all' : a.status)}
                      className={`px-2 py-0.5 rounded text-xs font-medium cursor-pointer hover:opacity-80 transition-opacity ${a.status === 'assigned' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}
                    >
                      {a.status}
                    </button>
                  </p>
                  {a.assignedTo && (
                    <p className="text-xs text-slate-400">
                      ...
                      <button
                        onClick={() =>
                          navigate(`/admin/employees/${a.assignedTo._id || a.assignedTo.id}`)
                        }
              ...
              <div className="flex gap-2">
                {a.status === 'available' && (
                  <Button variant="primary" size="sm" onClick={() => setAssignId(a._id)}>
                    Assign
                  </Button>
                )}
                {a.status === 'assigned' && (
                  <Button variant="outline" size="sm" onClick={() => returnAsset(a._id)}>
                    <BsArrowReturnLeft className="mr-1" />
                    Return
                  </Button>
                )}
              </div>
