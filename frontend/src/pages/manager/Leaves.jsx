...
const ManagerLeaves = () => {
  ...
  const allLeavesColumns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <span
          className={`rounded-full px-2 py-1 text-xs font-medium ${getStatusBadgeColor(row.status)}`}
        >
    ...
    <DashboardLayout>
      <div className="page-header mb-6">
        ...
        <p className="page-subtitle">
          Review team approvals, view company leaves, or submit a request.
        </p>
      </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-medium ${getStatusBadgeColor(leave.status)}`}
                        >
            ...
            <form onSubmit={handleRequestSubmit} className="space-y-4">
              ...
              <div className="form-group">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Medical certificates are required for sick leave &gt; 2 days.
                </p>
              </div>
              <div className="pt-4">
        ...
        faqs={[
          {
            ...
            a: 'Leave balance is based on annual entitlement minus approved leave days. Check individual employee profiles for current balances.',
          },
          {
            q: 'Can an employee request leave with insufficient balance?',
            a: 'Yes, but it requires manager approval. The system will warn about negative balance.',
          },
          {
            q: 'What happens when leave is approved?',
            a: "The employee's leave balance is reduced and the dates are marked as leave in attendance records.",
          },
          {
            q: 'Can I cancel an approved leave?',
            a: 'Only before the leave start date. Contact HR for cancellations of in-progress leave.',
          },
        ]}
        fetchStatus={async () => {
          const items = [];
          try {
            ...
            if (pending.length > 0)
              items.push({
                ...
                detail: 'Review and approve or reject pending requests in the Approvals tab.',
              });
            ...
            if (onLeaveToday.length > 0)
              items.push({
                level: 'info',
                message: `${onLeaveToday.length} employee${onLeaveToday.length > 1 ? 's are' : ' is'} on leave today`,
                detail: 'Plan coverage for absent employees.',
              });
            const lowBalance = emps.filter(e => e.leave_balance < 5);
            if (lowBalance.length > 0)
              items.push({
                level: 'warn',
                message: `${lowBalance.length} employee${lowBalance.length > 1 ? 's have' : ' has'} low leave balance (< 5 days)`,
                detail: 'Consider planning leave allocation for these employees.',
              });
            if (items.length === 0)
              items.push({
                level: 'success',
                message: 'Leave management is up to date — no issues found.',
              });
          } catch {
            items.push({
              level: 'info',
