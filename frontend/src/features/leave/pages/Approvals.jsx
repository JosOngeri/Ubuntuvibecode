...
export default function LeaveApprovals() {
  ...
  const employeeNameMap = useMemo(() => {
    return employees.reduce((accumulator, employee) => {
      accumulator[String(employee.id || employee._id)] = [employee.firstName, employee.lastName]
        .filter(Boolean)
        .join(' ');
      return accumulator;
    }, {});
  ...
  const pendingLeaves = useMemo(() => {
    return leaves.filter(
      leave =>
        approvedStatuses.includes(String(leave.status)) &&
        leaveKinds.includes(String(leave.type).toLowerCase())
    );
  }, [leaves]);
    ...
    <DashboardLayout>
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">
            Leave and Off-days Approvals
          </h1>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Review annual, sick, and off-day requests for your team, with conflict visibility for
      ...
      {conflictMap && Object.keys(conflictMap).length > 1 && (
        <div className="mb-6 rounded-2xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-900 dark:border-yellow-900 dark:bg-yellow-950 dark:text-yellow-200">
          <strong>Conflict Warning:</strong> multiple team members are off on overlapping dates.
          Review coverage before approving.
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            No pending annual or sick leave requests at the moment.
          </p>
        </div>
      ) : (
                  ...
                  {pendingLeaves.map(leave => (
                    <tr key={leave.id} className="align-top">
                      <td className="px-4 py-4 text-slate-950 dark:text-white">
                        ...
                        >
                          {employeeNameMap[String(leave.employee_id)] ||
                            leave.employee_name ||
                            `Employee ${leave.employee_id}`}
                        </button>
                      </td>
                      <td className="px-4 py-4 capitalize text-slate-700 dark:text-slate-300">
                        {leave.type}
                      </td>
          ...
          <aside className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            {selectedLeave ? (
              <div className="space-y-4 text-sm text-slate-700 dark:text-slate-300">
                <div>
                  ...
                  <div className="mt-1 font-medium text-slate-950 dark:text-white">
                    {employeeNameMap[String(selectedLeave.employee_id)] ||
                ...
                <div>
                  ...
                  <div className="mt-1 rounded-xl bg-slate-50 p-3 dark:bg-slate-950">
                ...
                {conflictMap[String(selectedLeave.id)] && (
                  <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-3 text-yellow-900 dark:border-yellow-900 dark:bg-yellow-950 dark:text-yellow-200">
                    Conflict warning: this request overlaps with{' '}
                    {conflictMap[String(selectedLeave.id)]} other pending request(s).
                  </div>
                )}
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Choose a request to review details.
              </p>
            )}
          </aside>
        </div>
