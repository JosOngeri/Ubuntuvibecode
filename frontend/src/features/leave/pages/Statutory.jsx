...
export default function LeaveStatutory({ standalone = true }) {
  ...
  const statutoryTypes = getLeaveTypes().filter(type =>
    ['maternity', 'paternity'].includes(type.toLowerCase())
  );
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
          Dedicated review view for long-term statutory leave requests with document preview and
          download support.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            No statutory maternity leave requests available right now.
          </p>
        </div>
      ) : (
            ...
            {statutoryLeaves.map(leave => (
              <button
                ...
                className={`w-full rounded-2xl border p-5 text-left shadow-sm transition ${
                  String(selectedId) === String(leave.id)
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            {selectedLeave ? (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                    Maternity leave is treated as statutory and the document preview is available
                    here.
                  </p>
                </div>
                ...
                {selectedLeave.attachment_path ? (
                  <div className="flex flex-wrap gap-3">
                    <button
                      onClick={() =>
                        window.open(
                          buildAttachmentUrl(selectedLeave.attachment_path),
                          '_blank',
                          'noopener,noreferrer'
                        )
                      }
              ...
              <div className="text-sm text-slate-500 dark:text-slate-400">
                Select a statutory leave request to inspect the uploaded medical document.
              </div>
            )}
          </aside>
        </div>
