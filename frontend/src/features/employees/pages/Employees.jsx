...
const Employees = ({ standalone = true }) => {
  ...
  const employmentTypeOptions = [
    ...new Set(employees.map(emp => emp.employmentType).filter(Boolean)),
  ];
  ...
  const filteredEmployees = employees
    .filter(emp => {
      ...
      const matchesRole =
        roleFilter === 'all' || (linkedUser?.role || '').toLowerCase() === roleFilter;
  ...
  const handleExportEmployeesReport = async () => {
    await downloadPdfReport({
      ...
      metadata: [
        ...
        {
          label: 'Type Filter',
          value: employmentTypeFilter === 'all' ? 'All' : employmentTypeFilter,
        },
      ],
    });
  };
          ...
          <div className="flex items-center gap-2">
            ...
            {canManageEmployees && (
              <button
                ...
                onClick={() => {
                  setChangingRoleFor({
                    userId: linkedUser._id || linkedUser.id,
                    username: linkedUser.username,
                  });
                  setNewRole(linkedUser.role);
                }}
              >
                change
              </button>
        ...
        return (
          <div className="flex gap-2">
            <Button
              size="sm"
              variant="primary"
              onClick={() => navigate(`/admin/employees/${rowId}`)}
            >
              View Details
            </Button>
            {canManageEmployees && (
    ...
    <div>
      ...
      <Card>
        <div className="employees-header flex flex-wrap items-end gap-3 mb-4">
          ...
          <Button variant="secondary" onClick={handleExportEmployeesReport}>
            Export Report
          </Button>
          {canManageEmployees && (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-900 rounded-lg p-6 w-full max-w-sm shadow-xl">
            ...
            <p className="text-sm text-slate-500 mb-4">
              User: <strong>{changingRoleFor.username}</strong>
            </p>
            <select
              value={newRole}
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={() => setChangingRoleFor(null)}>
                Cancel
              </Button>
            </div>
          </div>
