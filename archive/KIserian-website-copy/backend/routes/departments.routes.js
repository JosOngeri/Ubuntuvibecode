...
router.post('/',
  ...
  async (req, res) => {
    try {
      const { name, description, head_id, category } = req.body;

      const query = `
        INSERT INTO departments (name, description, head_id, category)
        VALUES ($1, $2, $3, $4)
        RETURNING *
      `;

      const result = await pool.query(query, [name, description, head_id, category]);
...
router.put('/:id',
  ...
  validate,
  validationRules.department.update,
  validate,
  async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const result = await pool.query(updateQuery, [name, description, head_id, category, id]);
