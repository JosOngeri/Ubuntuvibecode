...
const DailyLabourerAttendance = () => {
  ...
  const fetchAttendance = async () => {
    try {
      const res = await api.get('/daily-labourers/attendance');
  ...
  const handleCheckIn = async () => {
    try {
      await api.post('/daily-labourers/attendance/punch', { action: 'checkIn' });
  ...
  const handleCheckOut = async () => {
    try {
      await api.post('/daily-labourers/attendance/punch', { action: 'checkOut' });
        ...
        {todayRecord ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-gray-600">Wage for Day</p>
                <p className="text-lg font-medium">KES {todayRecord.wageForDay || 0}</p>
              </div>
            </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Wage</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {attendance.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-gray-500">{record.assignedTo}</td>
                  <td className="px-6 py-4 text-gray-500">KES {record.wageForDay || 0}</td>
...
export default DailyLabourerAttendance;
